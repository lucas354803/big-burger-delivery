...
const adicionais = [
  { nome:'Bacon em cubos', preco:5 },
  { nome:'Calabresa', preco:5 },
  { nome:'Queijo mussarela', preco:3 },
  { nome:'Cheddar extra', preco:4 },
  { nome:'Onion rings', preco:5 },
  { nome:'Maionese da casa', preco:2 }
];
let carrinho=[];
...
function render(){
  ...
  cart.innerHTML = carrinho.length ? carrinho.map((p,idx)=>`
    <div class="row">
      <span><b>1x ${p.nome}</b><br><small class="small">${(p.addons||[]).map(a=>a.nome).join(', ') || 'Sem adicionais'}</small></span>
      <b>${fmt(p.preco)}</b>
    </div>
  `).join('') : '<div class="cart-empty">🛒<br>Seu carrinho está vazio<br><small>Adicione itens deliciosos para começar!</small></div>';
}
...
function abrirProduto(i){
  ...
  addons.innerHTML = adicionais.map((a,idx)=>`
    <div class="addon" id="addon-${idx}">
      <span>${a.nome}<br><small>+ ${fmt(a.preco)}</small></span>
      <button onclick="toggleAddon(${idx})">+</button>
    </div>
  `).join('');
  modal.hidden=false;
}
...
function toggleAddon(idx){
  ...
  else if(addonsSelecionados.length<6) addonsSelecionados.push(idx);
...
function adicionarModal(){
  ...
  carrinho.push({ nome:p.nome, descricao:p.descricao, preco:p.preco+precoAdds, addons:adds });
  fecharModal();
  render();
}
async function finalizar(){
  ...
  try{
    ...
    if(data.pix?.qr_code_base64){
      result.innerHTML=`<div class="ok"><h3>Pix gerado!</h3><img class="qr" src="data:image/png;base64,${data.pix.qr_code_base64}"><textarea readonly>${data.pix.pix_copia_cola}</textarea><p class="small">Após pagar, o webhook libera a entrega automaticamente.</p></div>`
    }else{
      result.innerHTML=`<div class="ok"><h3>Pedido criado</h3><p>${data.mensagem||'Pedido registrado com sucesso.'}</p></div>`
    }
