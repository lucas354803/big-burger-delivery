...
function instanceCandidates(configured){
...
function getInstanceNameFromItem(item){
  return item?.name || item?.instanceName || item?.instance?.instanceName || item?.instance?.name || item?.id || '';
}
...
async function resolveEvolutionInstance(){
  ...
  const candidates = instanceCandidates(instance);
  const instances = await fetchEvolutionInstances();
  const names = instances.map(getInstanceNameFromItem).filter(Boolean);

  // Prefer a connected/open instance returned by Evolution.
  const connected = instances.find((i) => {
    const st = getInstanceStateFromItem(i);
    return /(open|connected|conectado)/i.test(st);
  });
  const connectedName = getInstanceNameFromItem(connected);

  return unique([connectedName, ...candidates, ...names]);
}

async function requestConnectionState(instanceName){
  const { baseUrl, apiKey } = evolutionEnv();
  const url = `${baseUrl}/instance/connectionState/${encodeURIComponent(instanceName)}`;
  const response = await fetch(url, { headers:{ apikey: apiKey } });
  const { data } = await readResponse(response);
  return { ok:response.ok, status:response.status, url, data, instance:instanceName };
}

export async function getEvolutionState(){
  const { baseUrl, apiKey } = evolutionEnv();
  if (!baseUrl || !apiKey) return { ok:false, error:'Evolution API não configurada.' };

  const tried = [];
  const names = await resolveEvolutionInstance();
  for (const name of names) {
    const state = await requestConnectionState(name).catch((e)=>({ ok:false, error:e.message, instance:name }));
    tried.push(state);
    if (state.ok) return { ...state, tried };
  }
  return tried[0] || { ok:false, error:'Nenhuma instância encontrada na Evolution.', tried };
}

async function postEvolutionText(instanceName, number, text){
  const { baseUrl, apiKey } = evolutionEnv();
  const url = `${baseUrl}/message/sendText/${encodeURIComponent(instanceName)}`;
  const payload = { number, text: String(text || ''), delay: 1200, linkPreview: false };
  ...
  return { ok:response.ok, status:response.status, instance:instanceName, number, url, payload, data, raw };
}
...
export async function sendEvolutionText(to, text){
  ...
  if (!baseUrl || !apiKey) {
    return { ok:false, skipped:true, error:'Evolution API não configurada no Vercel.', env:{ baseUrl_ok:Boolean(baseUrl), apiKey_ok:Boolean(apiKey) } };
  }
  if (!number) {
  ...
  const names = unique([state?.instance, ...(await resolveEvolutionInstance())]);
  const attempts = [];

  for (const name of names) {
    const sent = await postEvolutionText(name, number, text).catch((e)=>({ ok:false, error:e.message, instance:name }));
    attempts.push(sent);
    if (sent.ok) return { ...sent, state, attempts };
    // If it is not instance-not-found, do not spam every candidate.
    if (sent.status && sent.status !== 404) break;
  }

  const last = attempts[attempts.length - 1] || {};
  const err = new Error(`Falha ao enviar WhatsApp pela Evolution: HTTP ${last.status || 'erro'}`);
  err.status = last.status || 500;
  err.data = last.data || last.error || null;
  err.url = last.url || null;
  err.number = number;
  err.state = state;
