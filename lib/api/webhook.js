...
import { handleEvolutionAutoReply } from '../evolution.js';
export default async function handler(req, res) {
  try {
    const evolutionReply = await handleEvolutionAutoReply(req.body || {}).catch((e)=>({ handled:true, error:e.message, detalhes:e.data || null }));
    if (evolutionReply?.handled) return res.status(200).json({ ok:true, evolution:evolutionReply });
