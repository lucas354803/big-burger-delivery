...
export default async function handler(req, res) {
  ...
  try {
    ...
    const atuais = await supabaseFetch(`pedidos?id=eq.${encodeURIComponent(id)}&select=id,status,forma_pagamento`, { method:'GET', headers:{ Prefer:'' } });
    const atual = Array.isArray(atuais) ? atuais[0] : null;
    if (!atual) return res.status(404).json({ ok:false, error:'Pedido não encontrado' });
    if (STATUS_AGUARDANDO_PIX.has(String(atual.status||'')) && status !== 'cancelado') {
