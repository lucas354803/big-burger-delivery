...
import { buildStatusMessage, sendEvolutionText, formatBrazilNumber } from '../evolution.js';
  ...
  try {
    ...
    if (status !== 'cancelado' && status !== 'nao_realizado') {
      try {
        ...
        const telefoneCliente = pedido.cliente_telefone || pedido.telefone_cliente || pedido.telefone || pedido.whatsapp || pedido.celular;
        whatsapp = await sendEvolutionText(telefoneCliente, mensagem);
        whatsapp.number = whatsapp.number || formatBrazilNumber(telefoneCliente);
        ...
        const telefoneCliente = pedido.cliente_telefone || pedido.telefone_cliente || pedido.telefone || pedido.whatsapp || pedido.celular;
        whatsapp = { ok:false, error: err.message, detalhes: err.data || null, number: formatBrazilNumber(telefoneCliente) };
        await supabaseFetch(`pedidos?id=eq.${encodeURIComponent(id)}`, {
          method:'PATCH',
