...
export default async function handler(req, res) {
  ...
  try {
    ...
    const numero = String(body.numero || body.numero_casa || body.numero_endereco || '').trim();
    const enderecoFinal = endereco || [rua && (numero ? `${rua}, Nº ${numero}` : rua), bairro, cidade].filter(Boolean).join(' - ');
    ...
    if (!cliente_nome || !cliente_telefone || !enderecoFinal || !itens.length || valor_total <= 0) {
