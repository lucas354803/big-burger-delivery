...
async function apagarPedidosDoCliente({ telefone, nome }) {
  const telLimpo = onlyDigits(telefone);
  const nomeLimpo = String(nome || '').trim().toLowerCase();
  if (!telLimpo && !nomeLimpo) return 0;

  const pedidos = await supabaseFetch('pedidos?select=id,cliente_nome,cliente_telefone,whatsapp,telefone,celular,cliente_whatsapp&limit=5000', {
    method: 'GET',
    headers: { Prefer: '' }
  });
  ...
  const ids = (Array.isArray(pedidos) ? pedidos : [])
    .filter(p => {
      const telefones = [p.cliente_telefone, p.whatsapp, p.telefone, p.celular, p.cliente_whatsapp].map(onlyDigits).filter(Boolean);
      const bateTelefone = telLimpo && telefones.includes(telLimpo);
      const bateNome = !telLimpo && nomeLimpo && String(p.cliente_nome || '').trim().toLowerCase() === nomeLimpo;
      return bateTelefone || bateNome;
    })
    .map(p => p.id)
    .filter(Boolean);
  ...
  for (let i = 0; i < ids.length; i += 80) {
    const lote = ids.slice(i, i + 80).map(id => `"${String(id).replaceAll('"', '\\"')}"`).join(',');
    await supabaseFetch(`pedidos?id=in.(${lote})`, { method: 'DELETE' });
  }
  return ids.length;
}
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'DELETE') {
      const id = req.query?.id || req.body?.id;
      const telefone = req.query?.telefone || req.body?.telefone || req.body?.cliente_telefone;
      const nome = req.query?.nome || req.body?.nome || req.body?.cliente_nome;
      const apagarPedidos = String(req.query?.apagarPedidos || req.body?.apagarPedidos || '').toLowerCase() === 'true';

      if (id) {
        await supabaseFetch(`clientes_cadastros?id=eq.${encodeURIComponent(id)}`, { method:'DELETE' });
      }

      let pedidos_apagados = 0;
      if (apagarPedidos) {
        pedidos_apagados = await apagarPedidosDoCliente({ telefone, nome });
      }

      if (!id && !apagarPedidos) return res.status(400).json({ ok:false, error:'Informe o ID do cliente ou marque apagarPedidos=true.' });
      return res.status(200).json({ ok:true, pedidos_apagados });
    }
