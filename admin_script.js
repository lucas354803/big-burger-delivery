...
function montarHtmlComandaQZ(p){
  ...
  const largura=papel===58 ? 384 : 576;
  const logoW=papel===58 ? 88 : 120;
  const qrW=papel===58 ? 96 : 118;
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    html,body{margin:0;padding:0;background:#fff;color:#000;font-family:Arial,'Segoe UI Emoji','Noto Color Emoji',Helvetica,sans-serif;-webkit-print-color-adjust:exact;print-color-adjust:exact}
...
async function imprimirPedidoQZ(p){
  ...
  // MODO IMAGEM/PIXEL: o QZ transforma a comanda HTML em imagem antes de enviar.
  // Isso evita os ????? quando o produto tem emoji ou caractere especial.
  const papel=Number(cfg.papel||80);
  const larguraPx = papel===58 ? 384 : 576;
  ...
  try{
    ...
    console.warn('Falha no QZ imagem/pixel, tentando HTML rasterizado:', pixelErr);
    await qz.print(config, [{type:'html', format:'plain', data:html}]);
  }
  const out=document.getElementById('impressoraStatus');
  if(out) out.innerHTML='✅ Comanda em modo imagem enviada direto para: '+escapeHtml(printer);
}
