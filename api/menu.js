...
const fallbackCategoriasComplementos = [
  { id:'carnes', nome:'🥩 Carnes', min_escolha:0, max_escolha:3, ordem:1, ativo:true },
  { id:'queijos', nome:'🧀 Queijos', min_escolha:0, max_escolha:3, ordem:2, ativo:true },
  { id:'extras', nome:'🍟 Extras', min_escolha:0, max_escolha:6, ordem:3, ativo:true },
  { id:'molhos', nome:'🥫 Molhos', min_escolha:0, max_escolha:4, ordem:4, ativo:true }
];
...
export default async function handler(req, res) {
  try {
    const [categorias, produtos, categorias_complementos, complementos] = await Promise.all([
    ...
    res.status(200).json({ ok:true, categorias, produtos, categorias_complementos, complementos });
  } catch (e) {
    res.status(200).json({ ok:true, fallback:true, categorias:fallbackCategorias, produtos:fallbackProdutos, categorias_complementos:fallbackCategoriasComplementos, complementos:fallbackComplementos });
  }
}
