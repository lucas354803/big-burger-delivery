...
module.exports = async function handler(req, res) {
  try {
    if (req.method === 'GET') {
      const corridas = await supabaseFetch('corridas?select=*,pedidos(*)&order=created_at.desc&limit=100', { method: 'GET', headers: { prefer: undefined } });
    ...
    if (acao === 'finalizar' && updated[0]?.pedido_id) await supabaseFetch(`pedidos?id=eq.${updated[0].pedido_id}`, { method: 'PATCH', body: JSON.stringify({ status: 'entregue' }) });
