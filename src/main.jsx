...
function Cliente({products,setOrders,open}){
 ...
 const finish=e=>{e.preventDefault(); const fd=new FormData(e.currentTarget); const order={id:'BB-'+Date.now().toString().slice(-6),created:new Date().toLocaleString('pt-BR'),status:'Em análise',items:cart, total, customer:Object.fromEntries(fd.entries())}; setOrders(o=>[order,...o]); setCart([]); setCheckout(false); setLast(order)};
...
function Admin({products,setProducts,orders,setOrders,open,setOpen}){
 ...
 return <main className="admin"><nav><button onClick={()=>setTab('pedidos')}>Pedidos</button><button onClick={()=>setTab('produtos')}>Produtos</button><button onClick={()=>setTab('relatorio')}>Relatório</button><button onClick={()=>setOpen(!open)}><Store/> {open?'Fechar loja':'Abrir loja'}</button></nav>{tab==='pedidos'&&<div className="columns">{statuses.map(st=><section><h2>{st} <small>{orders.filter(o=>o.status===st).length}</small></h2>{orders.filter(o=>o.status===st).map(o=><div className="order"><h3>{o.id} - {money(o.total)}</h3><p><b>{o.customer.nome}</b> - {o.customer.whatsapp}<br/>{o.customer.rua} {o.customer.bairro}<br/>{o.customer.pagamento} • {o.created}</p>{o.items.map(i=><p>{i.qty}x {i.name}</p>)}<select value={o.status} onChange={e=>update(o.id,e.target.value)}>{statuses.map(s=><option>{s}</option>)}</select><button onClick={()=>window.print()}><Printer/> Imprimir</button><button onClick={()=>del(o.id)}><Trash2/> Apagar</button></div>)}</section>)}</div>}{tab==='produtos'&&<Products products={products} setProducts={setProducts}/>} {tab==='relatorio'&&<Report orders={orders}/>}</main>
}
